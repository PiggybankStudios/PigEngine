...
// |                             help                             |
// +--------------------------------------------------------------+
#define Debug_Help_Def "void help(string command)"
#define Debug_Help_Desc "Displays this list of commands"
void Debug_Help(MyStr_t commandStr)
{
...
EXPRESSION_FUNC_DEFINITION(Debug_Help_Glue) { EXP_GET_ARG_STR(0, commandStr); Debug_Help(commandStr); return NewExpValueVoid(); }
...
bool PigHandleDebugCommand(MyStr_t command, u64 numArguments, MyStr_t* arguments, MyStr_t fullInputStr)
{
	...
	// |             run              |
	// +==============================+
	else if (StrCompareIgnoreCase(command, "run") == 0)
	{
		if (numArguments < 1) { PrintLine_E("Expected at least 1 argument, not %llu", numArguments); return validCommand; }
		
		MemArena_t* scratch = GetScratchArena();
		
		MyStr_t expressionStr = fullInputStr;
		TrimWhitespace(&expressionStr);
		if (StrStartsWith(expressionStr, "run")) { expressionStr.chars += 3; expressionStr.length -= 3; }
		TrimWhitespace(&expressionStr);
		...
		// r32 foo = 14.123f;
		// AddExpVariableDefR32(&context, "foo", &foo);
		
		ExpValue_t result = {};
		MyStr_t errorStr = TryRunExpressionErrorStr(expressionStr, scratch, &result, &context);
		if (IsEmptyStr(errorStr))
		{
		...
		{
			PrintLine_W("%.*s in \"%.*s\"", StrPrint(errorStr), StrPrint(expressionStr));
		}
