/*
File:   pig_audio_debug.cpp
Author: Taylor Robbins
Date:   01\15\2022
Description: 
	** Holds a bunch of functions that help us manage and render
	** the debug visual for the audio output (PigAudioOutGraph_t)
*/
...
#define AUDIO_OUT_GRAPH_LINE_GRAPH_SCALE 0.4 //scale (anything scale greater than this will be drawn as a line graph)
...
void InitPigAudioOutGraph(PigAudioOutGraph_t* graph)
{
	...
	graph->enabled = SHOW_AUDIO_OUT_GRAPH_ON_STARTUP;
}
...
void UpdatePigAudioOutGraph(PigAudioOutGraph_t* graph)
{
	...
	// +==============================+
	if (KeyPressed(Key_F3) && KeyDownRaw(Key_Shift) && KeyDownRaw(Key_Control))
	{
		HandleKeyExtended(Key_F3);
...
void RenderPigAudioOutGraph(PigAudioOutGraph_t* graph)
{
	...
	if (graph->enabled)
	{
		...
		#endif
		// RcDrawTextPrint(NewVec2(graph->mainRec.x, graph->mainRec.y + graph->mainRec.height + RcGetLineHeight()*2), White, "Scale %.3f -> %.3f (min %f)", graph->scale, graph->scaleGoto, graph->minScale);
		// RcDrawTextPrint(NewVec2(graph->mainRec.x, graph->mainRec.y + graph->mainRec.height + RcGetLineHeight()*3), White, "ViewCenter %.3f -> %.3f", graph->viewCenter, graph->viewCenterGoto);
		...
		// +==============================+
		{
			...
			r32 bufferLeft = ConvertSampleIndexToScreenSpace(graph, 0);
		...
		if (graph->scale < AUDIO_OUT_GRAPH_LINE_GRAPH_SCALE)
		{
